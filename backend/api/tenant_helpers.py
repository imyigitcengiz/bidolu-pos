"""Tenant sahiplik doğrulama ve rol yönetimi."""

from rest_framework import status
from rest_framework.response import Response

STORE_OWNER_ASSIGNABLE_ROLES = frozenset({
    'manager', 'waiter', 'cashier', 'kitchen',
})
SUPER_ADMIN_ASSIGNABLE_ROLES = frozenset({
    'super_admin', 'store_owner', 'manager', 'waiter', 'cashier', 'kitchen',
})


def get_user_brand(user):
    profile = getattr(user, 'profile', None)
    if profile and profile.role == 'super_admin':
        return None
    return profile.brand if profile else None


def validate_role_assignment(caller_role, new_role):
    """Rol ataması izinli mi — (ok, error_message)."""
    if caller_role == 'super_admin':
        allowed = SUPER_ADMIN_ASSIGNABLE_ROLES
    elif caller_role == 'store_owner':
        allowed = STORE_OWNER_ASSIGNABLE_ROLES
    else:
        return False, 'Rol atama yetkiniz yok.'
    if new_role not in allowed:
        return False, f'"{new_role}" rolünü atama yetkiniz yok.'
    return True, None


def user_owns_brand(user, brand):
    if brand is None:
        return False
    profile = getattr(user, 'profile', None)
    if not profile:
        return False
    if profile.role == 'super_admin':
        return True
    return profile.brand_id == brand.id


def deny_cross_tenant(user, obj_brand):
    if obj_brand is None:
        return Response({'error': 'Kayıt marka bilgisi içermiyor.'}, status=status.HTTP_400_BAD_REQUEST)
    if not user_owns_brand(user, obj_brand):
        return Response({'error': 'Bu kaynağa erişim yetkiniz yok.'}, status=status.HTTP_403_FORBIDDEN)
    return None


def get_tenant_table(user, table_id):
    from .models import Table
    try:
        table = Table.objects.select_related('brand').get(id=table_id)
    except Table.DoesNotExist:
        return None, Response({'error': 'Masa bulunamadı'}, status=status.HTTP_400_BAD_REQUEST)
    err = deny_cross_tenant(user, table.brand)
    if err:
        return None, err
    return table, None


def get_tenant_menu_item(user, menu_item_id):
    from .models import MenuItem
    try:
        item = MenuItem.objects.select_related('brand').get(id=menu_item_id)
    except MenuItem.DoesNotExist:
        return None, Response({'error': 'Menü ürünü bulunamadı'}, status=status.HTTP_400_BAD_REQUEST)
    err = deny_cross_tenant(user, item.brand)
    if err:
        return None, err
    return item, None


def get_tenant_ingredient(user, ingredient_id):
    from .models import Ingredient
    try:
        ing = Ingredient.objects.select_related('brand').get(id=ingredient_id)
    except Ingredient.DoesNotExist:
        return None, Response({'error': 'Malzeme bulunamadı'}, status=status.HTTP_400_BAD_REQUEST)
    err = deny_cross_tenant(user, ing.brand)
    if err:
        return None, err
    return ing, None


def get_tenant_order(user, order_id):
    from .models import Order
    try:
        order = Order.objects.select_related('brand').get(id=order_id)
    except Order.DoesNotExist:
        return None, Response({'error': 'Sipariş bulunamadı'}, status=status.HTTP_400_BAD_REQUEST)
    err = deny_cross_tenant(user, order.brand)
    if err:
        return None, err
    return order, None


def get_tenant_register(user, register_id):
    from .models import CashRegister
    try:
        reg = CashRegister.objects.select_related('brand').get(id=register_id)
    except CashRegister.DoesNotExist:
        return None, Response({'error': 'Kasa bulunamadı'}, status=status.HTTP_400_BAD_REQUEST)
    err = deny_cross_tenant(user, reg.brand)
    if err:
        return None, err
    return reg, None
