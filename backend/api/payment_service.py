...
from .models import Brand, Invoice, RestaurantProfile
from .plan_limits import BILLING_CYCLE_DAYS, PLAN_LIMITS


def get_active_provider():
    """Yapılandırılmış ödeme sağlayıcısını döner; prod'da mock yasak."""
...
def fulfill_subscription_payment(invoice):
    """Ödeme onaylandığında planı aktifleştirir."""
    if invoice.paid:
        return invoice
    brand = invoice.brand
    brand.plan = invoice.plan
    today = timezone.localdate()
    if brand.plan_expiry and brand.plan_expiry >= today:
        brand.plan_expiry = brand.plan_expiry + timedelta(days=BILLING_CYCLE_DAYS)
    else:
        brand.plan_expiry = today + timedelta(days=BILLING_CYCLE_DAYS)
...
def _create_stripe_checkout(invoice, brand, user):
    ...
    payload = {
        ...
        'line_items[0][price_data][product_data][name]': f'BiDolu POS — {plan_label} Plan (Aylık)',
...
def _create_iyzico_checkout(invoice, brand, user):
    ...
    payload = {
        ...
        'callbackUrl': f'{backend}/api/payments/iyzico/callback/',
        ...
        'buyer': {
            ...
            'email': user.email or f'{user.username}@bidolupos.local',
        ...
        'basketItems': [{
            ...
            'name': f'BiDolu POS {plan_label} Abonelik',
            'category1': 'SaaS',
            'itemType': 'VIRTUAL',
            'price': str(invoice.amount),
        }],
...
def initiate_checkout(brand, plan, user):
    """Ödeme oturumu başlatır; mock modda anında tamamlar."""
...
def verify_stripe_session(session_id, user=None):
    ...
    if user is not None:
        from .tenant_helpers import user_owns_brand
        profile = getattr(user, 'profile', None)
        if profile and profile.role != 'super_admin':
            if not user_owns_brand(user, invoice.brand):
